...
from eureka.core.configuration import Configurable, UserConfigurable
from eureka.core.resource.model_providers.schema import (
...
from eureka.core.utils.json_utils import json_loads
...
TOKENIZER_ENCODING_BY_MODEL_PREFIX = {
    "gpt-5": "o200k_base",
    "o1": "o200k_base",
    "o3": "o200k_base",
    "o4": "o200k_base",
...
OPEN_AI_CHAT_MODELS = {
    ...
    for info in [
        ...
        ChatModelInfo(
            name=OpenAIModelName.GPT5_5,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.OPENAI,
            prompt_token_cost=5.0 / 1_000_000,
        ...
        ChatModelInfo(
            ...
            completion_token_cost=15.0 / 1_000_000,
            max_tokens=1_000_000,
            has_function_call_api=True,
        ),
        ChatModelInfo(
        ...
        ChatModelInfo(
            name=OpenAIModelName.DEEPSEEK_V4_FLASH,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.OPENAI,
            prompt_token_cost=0.14 / 1_000_000,
            completion_token_cost=0.28 / 1_000_000,
            max_tokens=1_000_000,
            has_function_call_api=True,
        ),
        ChatModelInfo(
            name=OpenAIModelName.DEEPSEEK_V4_PRO,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.OPENAI,
            prompt_token_cost=0.435 / 1_000_000,
            completion_token_cost=0.87 / 1_000_000,
            max_tokens=1_000_000,
            has_function_call_api=True,
        ),
        ChatModelInfo(
            name=OpenAIModelName.DEEPSEEK_R1,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.OPENAI,
            prompt_token_cost=0.14 / 1000000,
            completion_token_cost=0.55 / 1000000,
            max_tokens=8192,
            has_function_call_api=False,
        ),
        ChatModelInfo(
            name=OpenAIModelName.DEEPSEEK_V3,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.OPENAI,
            prompt_token_cost=0.14 / 1000000,
            completion_token_cost=0.55 / 1000000,
...
chat_model_mapping = {
    ...
    OpenAIModelName.GPT5_5: [OpenAIModelName.GPT5],
    OpenAIModelName.DEEPSEEK_V4_FLASH: [
        OpenAIModelName.DEEPSEEK_CHAT,
        OpenAIModelName.DEEPSEEK_REASONER,
    ],
}
...
def _uses_max_completion_tokens(model_name: str) -> bool:
    return str(model_name).startswith(MAX_COMPLETION_TOKENS_MODEL_PREFIXES)


@functools.lru_cache(maxsize=None)
def _chat_completions_create_accepts_kwarg(argument: str) -> bool:
    from openai.resources.chat.completions import AsyncCompletions

    return argument in inspect.signature(AsyncCompletions.create).parameters
...
class OpenAICredentials(ModelProviderCredentials):
    ...
    api_key: SecretStr = UserConfigurable(
        from_env=lambda: os.getenv("OPENAI_API_KEY") or os.getenv("DEEPSEEK_API_KEY")
    )
    api_base: Optional[SecretStr] = UserConfigurable(
        ...
        from_env=lambda: (
            ...
            or (
                DEFAULT_DEEPSEEK_API_BASE_URL if os.getenv("DEEPSEEK_API_KEY") else None
            )
        ),
    )
...
class OpenAIProvider(
    ...
    def get_tokenizer(cls, model_name: OpenAIModelName) -> ModelTokenizer:
        ...
        for model_prefix, encoding_name in TOKENIZER_ENCODING_BY_MODEL_PREFIX.items():
        ...
        try:
            ...
            logging.getLogger(__class__.__name__).debug(
                "Tokenizer for model %s was not found. Defaulting to o200k_base.",
                model_name,
            )
            try:
    ...
    def count_tokens(cls, text: str, model_name: OpenAIModelName) -> int:
        if model_name.startswith("deepseek"):
            chat_tokenizer_dir = "./eureka/core/resource/model_providers"
    ...
    def count_message_tokens(
        ...
        elif model_name.startswith(("gpt-4", "gpt-5", "o1", "o3", "o4")):
        ...
        try:
            if encoding_model == "deepseek":
                chat_tokenizer_dir = "./eureka/core/resource/model_providers"
                encoding = transformers.AutoTokenizer.from_pretrained(
                    chat_tokenizer_dir, trust_remote_code=True
                )
            else:
                encoding = cls.get_tokenizer(encoding_model)
        except KeyError:
    ...
    def _get_completion_kwargs(
        ...
        **kwargs,
        ...
        if _uses_max_completion_tokens(model_name):
            ...
            if not _chat_completions_create_accepts_kwarg("max_completion_tokens"):
