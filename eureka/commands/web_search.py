...
def web_search(query: str, agent: Agent, num_results: int = 8) -> str:
    ...
    try:
        ...
        if (
            agent.legacy_config.google_api_key
            and agent.legacy_config.google_custom_search_engine_id
        ):
            try:
                return google(query, agent=agent, num_results=num_results)
            except Exception as google_error:
                return _search_unavailable_message(query, e, google_error)

        return _search_unavailable_message(query, e)
...
def _search_unavailable_message(
    query: str,
    duckduckgo_error: Exception,
    google_error: Exception | None = None,
) -> str:
    message = (
        "## Search unavailable\n"
        f"Web search for {query!r} could not be completed because DuckDuckGo "
        f"returned an error: {duckduckgo_error}."
    )
    if google_error:
        message += f"\nGoogle fallback also failed: {google_error}."
    else:
        message += (
            "\nConfigure GOOGLE_API_KEY and GOOGLE_CUSTOM_SEARCH_ENGINE_ID to enable "
            "the Google fallback, or try again later."
        )
    return safe_google_results(message)
...
def google(query: str, agent: Agent, num_results: int = 8) -> str | list[str]:
    ...
    try:
        ...
        import httplib2
        http = httplib2.Http(proxy_info=httplib2.ProxyInfo(httplib2.socks.PROXY_TYPE_HTTP,"127.0.0.1",7890
        ))
