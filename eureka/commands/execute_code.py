...
from eureka.agents.utils.exceptions import (
    CodeExecutionError,
...
from eureka.core.utils.json_schema import JSONSchema  # 新增
...
def execute_python_file(
    ...
    if we_are_running_in_a_docker_container():
        logger.debug(
            "Eureka is running in a Docker container; "
    ...
    logger.debug("Eureka is not running in a Docker container")
    try:
        ...
        image_name = "syrsteven/autods:0.1.0"
        ...
        try:
            ...
            logger.info(f"Creating new {image_name} container...")
            container: DockerContainer = client.containers.run(
                image_name,
                # ["sleep", "600"],  # Max 60 seconds to prevent permanent hangs
                [
                    "python",
                    "-B",
                ...
                runtime="nvidia",  # 新增
                # network="host",
                environment=environment_variables,  # 新增
            )  # type: ignore
        ...
        # logger.info(f"Running {file_path} in container {container.name}...")
        # exec_result = container.run(
        #     [
        #         "python",
        #         "-B",
        #         file_path.relative_to(agent.workspace.root).as_posix(),
        ...
        container.remove()
        # with open('/data/yuanrui/logs0.txt','w') as f:
        #     f.write(logs)

        def process_string_with_backspaces(input_string):
            processed_string = []
        ...
        # 生成文件�?        # log_filename = f'/data/yuanrui/logs/eureka/log_{current_time}.txt'

        # 写入日志文件
        logs1 = remove_empty_lines(process_string_with_backspaces(logs))
        # logs2 = re.sub(r'\[.*?\]', '', logs1)
        # with open(log_filename, 'w') as f:
        #     f.write(logs1)

        # print(logs)

        # with open('/data/yuanrui/eureka/logs1.txt','w') as f:
        #     f.write(logs1)

        # if exec_result.exit_code != 0:
        #     raise CodeExecutionError(logs1)

        return logs1
    # remove_empty_lines(process_string_with_backspaces(logs))##已修�?
