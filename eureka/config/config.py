...
import eureka
from eureka.core.configuration.schema import (
    Configurable,
    SystemSettings,
    UserConfigurable,
)
...
from eureka.file_storage import FileStorageBackendName
from eureka.logs.config import LoggingConfig
from eureka.plugins.plugins_config import PluginsConfig
from eureka.speech import TTSConfig

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(eureka.__file__).parent.parent
...
class Config(SystemSettings, arbitrary_types_allowed=True):
    name: str = "Eureka configuration"
    description: str = "Default configuration for the Eureka application."
    ...
    memory_index: str = UserConfigurable("eureka-memory", from_env="MEMORY_INDEX")
...
def assert_config_has_openai_api_key(config: Config) -> None:
    """Warn if the OpenAI API key is missing or still the template placeholder."""
    openai_api_key = (
    ...
    if not openai_api_key:
        logger.warning(
            "OpenAI API key is not set; startup will continue, but OpenAI calls "
    ...
    if openai_api_key == "your-openai-api-key":
        logger.warning(
            "Invalid OpenAI API key placeholder detected; startup will continue, "
            "but OpenAI calls will fail until OPENAI_API_KEY is configured."
        )
        return
