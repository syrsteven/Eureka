...
from eureka.core.resource.model_providers.schema import ChatModelInfo
from eureka.singleton import Singleton
    ...
    def get_models(self, openai_credentials: OpenAICredentials) -> List[Model]:
        ...
        try:
            ...
            self.models = [
                ...
                if model.id.startswith(("gpt", "o", "deepseek"))
            ]
        except APIError as e:
            logger.warning(
                "Could not fetch OpenAI models: %s. Falling back to local model "
                "metadata; model calls may still fail until credentials are valid.",
                e.message,
            )
            self.models = [
                Model(id=model_name, created=0, object="model", owned_by="local")
