...
from eureka.agent_factory.configurators import configure_agent_with_state
from eureka.agent_factory.generators import generate_agent_for_task
from eureka.agent_manager import AgentManager
from eureka.agents.utils.exceptions import AgentFinished
from eureka.app.utils import is_port_free
from eureka.commands.system import finish
from eureka.commands.user_interaction import ask_user
from eureka.config import Config
from eureka.core.resource.model_providers import ChatModelProvider
from eureka.core.resource.model_providers.openai import OpenAIProvider
from eureka.core.resource.model_providers.schema import ModelProviderBudget
from eureka.file_storage import FileStorage
from eureka.logs.utils import fmt_kwargs
from eureka.models.action_history import ActionErrorResult, ActionSuccessResult
...
class AgentProtocolServer:
    ...
    async def start(self, port: int = 8000, router: APIRouter = base_router):
        ...
        app = FastAPI(
            title="Eureka Server",#存疑
            description="Forked from Eureka Forge; "
        ...
        logger.info(f"Eureka server starting on http://localhost:{port}")
    ...
    def _get_task_llm_provider(
        ...
        if task.additional_input and (user_id := task.additional_input.get("user_id")):
            _extra_request_headers["Eureka-UserID"] = user_id #存疑
