"""Main script for the Eureka package."""
...
@click.option("--gpt4o_only", is_flag=True, help="Enable GPT-4o Only Mode")
@click.option("--gpt5_5", is_flag=True, help="Enable GPT-5.5 Only Mode")
@click.option("--gpt5_4", is_flag=True, help="Enable GPT-5.4 Only Mode")
@click.option("--gpt5_4_mini", is_flag=True, help="Enable GPT-5.4 mini Only Mode")
@click.option(
    "--deepseek_v4_flash", is_flag=True, help="Enable deepseek-v4-flash Only Mode"
)
@click.option(
    "--deepseek_v4_pro", is_flag=True, help="Enable deepseek-v4-pro Only Mode"
)
@click.option("--deepseek_r1", is_flag=True, help="Enable deepseek-r1 Only Mode")
@click.option("--deepseek_v3", is_flag=True, help="Enable deepseek-v3 Only Mode")
@click.option(
    "-b",
    "--browser-name",
...
@click.option(
    "--allow-downloads",
    is_flag=True,
    help="Dangerous: Allows Eureka to download files natively.",
)
@click.option(
...
@click.option(
    ...
    help=(
        "Specifies which ai_settings.yaml file to use, relative to the Eureka"
...
def run(
    ...
    from eureka.app.main import run_eureka
...
def serve(
    ...
    """
    Starts an Agent Protocol compliant Eureka server, which creates a custom agent for
    every task.
    """
    # Put imports inside function to avoid importing everything when starting the CLI
    from eureka.app.main import run_eureka_server
