...
def clean_input(config: "Config", prompt: str = ""):
    try:
        ...
        logger.info("You interrupted Eureka")
...
def get_bulletin_from_web():
    try:
        response = requests.get(
            "https://raw.githubusercontent.com/Significant-Gravitas/eureka/master/Eurekas/eureka/BULLETIN.md"  # noqa: E501
        )
...
def vcs_state_diverges_from_master() -> bool:
    ...
    """
    return False
    # paths_we_care_about = "Eurekas/eureka/eureka/**/*.py"
    # try:
    #     repo = Repo(search_parent_directories=True)
    ...
    #                 for path in (
    #                     f"Significant-Gravitas/{n}.git" for n in ("Eureka", "Auto-GPT")
    #                 )
    #             )
    #         ):
    #             og_remote = remote
    #             break
    #     else:
    #         # Original Eureka remote is not configured: assume local codebase diverges
    #         return True

    #     master_branch = og_remote.refs.master
    #     with contextlib.suppress(StopIteration):
    #         next(repo.iter_commits(f"HEAD..{master_branch}", paths=paths_we_care_about))
    #         # Local repo is one or more commits ahead of OG Eureka master branch
    #         return True
...
def get_latest_bulletin() -> tuple[str, bool]:#NEW
    ...
    news_header = Fore.YELLOW + "Welcome to Eureka!\n"
    if new_bulletin or current_bulletin:
        news_header += (
            "Below you'll find the latest Eureka News and feature updates!\n"
            "If you don't wish to see this message, you "
            "can run Eureka with the *--skip-news* flag.\n"
        )
...
### PLEASE READ THIS DISCLAIMER AND INDEMNIFICATION AGREEMENT CAREFULLY BEFORE USING THE Eureka SYSTEM. BY USING THE Eureka SYSTEM, YOU AGREE TO BE BOUND BY THIS AGREEMENT.

## Introduction
Eureka (the "System") is a project that connects a GPT-like artificial intelligence system to the internet and allows it to automate tasks. While the System is designed to be useful and efficient, there may be instances where the System could perform actions that may cause harm or have unintended consequences.

## No Liability for Actions of the System
The developers, contributors, and maintainers of the Eureka project (collectively, the "Project Parties") make no warranties or representations, express or implied, about the System's performance, accuracy, reliability, or safety. By using the System, you understand and agree that the Project Parties shall not be liable for any actions taken by the System or any consequences resulting from such actions.
...
def print_motd(config: "Config", logger: logging.Logger): #close news
...
def print_python_version_info(logger: logging.Logger):
    if sys.version_info < (3, 10):
        logger.error(
            ...
            "parts of Eureka with this version. "
