...
from eureka.agent_factory.configurators import configure_agent_with_state, create_agent
from eureka.agent_factory.profile_generator import generate_agent_profile_for_task
from eureka.agent_manager import AgentManager
from eureka.agents import AgentThoughts, CommandArgs, CommandName
from eureka.agents.utils.exceptions import AgentTerminated, InvalidAgentResponseError
from eureka.commands.execute_code import (
    is_docker_available,
    we_are_running_in_a_docker_container,
)
from eureka.commands.system import finish
...
from eureka.core.resource.model_providers.openai import OpenAIProvider
from eureka.core.runner.client_lib.utils import coroutine
from eureka.file_storage import FileStorageBackendName, get_storage
from eureka.logs.config import configure_chat_plugins, configure_logging
from eureka.logs.helpers import print_attribute, speak
from eureka.models.action_history import ActionInterruptedByHuman
from eureka.plugins import scan_plugins
...
async def run_eureka(
    ...
    if not agent:
        ...
        while task.strip() == "":
            task = clean_input(
                config,
                "Enter the task that you want Eureka to execute,"
...
async def run_interaction_loop(
    ...
    def graceful_agent_interrupt(signum: int, frame: Optional[FrameType]) -> None:
        ...
        if cycles_remaining in [0, 1]:
            ...
            logger.warning("Press Ctrl+C again if you want to stop Eureka immediately.")
