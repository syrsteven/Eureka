...
import eureka.app.utils
from eureka.app.utils import (
    get_bulletin_from_web,
    get_current_git_branch,
    get_latest_bulletin,
    set_env_config_value,
)
from eureka.core.utils.json_utils import extract_dict_from_json
from eureka.utils import validate_yaml_file
...
def test_get_latest_bulletin_with_file():
    ...
    with patch("eureka.app.utils.get_bulletin_from_web", return_value=""):
        bulletin, is_new = get_latest_bulletin()
        assert expected_content in bulletin
...
def test_get_latest_bulletin_with_new_bulletin():
    ...
    with patch(
        "eureka.app.utils.get_bulletin_from_web", return_value=expected_content
    ):
        bulletin, is_new = get_latest_bulletin()
...
def mock_env_file(mock_env_file_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(eureka.app.utils, "ENV_FILE_PATH", mock_env_file_path)
