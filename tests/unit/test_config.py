...
from eureka.app.configurator import GPT_3_MODEL, GPT_4_MODEL, apply_overrides_to_config
from eureka.config import Config, ConfigBuilder, assert_config_has_openai_api_key
...
def test_fallback_to_gpt3_if_gpt4_not_available(
    ...
    mock_list_models.return_value = SyncPage(
        data=[Model(id=GPT_3_MODEL, created=0, object="model", owned_by="Eureka")],
...
def test_missing_openai_api_key_does_not_exit(config: Config, caplog) -> None:
    config.openai_credentials = None

    assert_config_has_openai_api_key(config)

    assert "OpenAI API key is not set" in caplog.text


def test_invalid_openai_api_key_does_not_exit(config: Config, caplog) -> None:
    assert config.openai_credentials is not None
    config.openai_credentials.api_key = SecretStr("your-openai-api-key")

    assert_config_has_openai_api_key(config)

    assert "Invalid OpenAI API key" in caplog.text


def test_get_models_falls_back_to_known_models_when_openai_fails(
    ...
    models = api_manager.get_models(config.openai_credentials)

    assert any(model.id == GPT_3_MODEL for model in models)
    assert "Could not fetch OpenAI models" in caplog.text
...
def test_create_config_gpt4only(config: Config) -> None:
    with mock.patch("eureka.llm.api_manager.ApiManager.get_models") as mock_get_models:
        mock_get_models.return_value = [
            Model(id=GPT_4_MODEL, created=0, object="model", owned_by="Eureka")
        ]
        apply_overrides_to_config(
            config=config,
            gpt4only=True,
