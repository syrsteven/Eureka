...
from eureka.agents.utils.exceptions import ConfigurationError
from eureka.commands.web_search import google, safe_google_results, web_search
...
def test_google_search(
    ...
    mocker.patch("eureka.commands.web_search.DDGS.text", mock_ddg)
...
def test_web_search_returns_unavailable_message_on_duckduckgo_rate_limit(
    mocker, agent: Agent
):
    mocker.patch(
        "eureka.commands.web_search.DDGS.text",
        side_effect=DuckDuckGoSearchException("Ratelimit"),
    )

    actual_output = web_search("test", agent=agent, num_results=1)

    assert "Search unavailable" in actual_output
    assert "Ratelimit" in actual_output


def test_web_search_uses_google_fallback_on_duckduckgo_rate_limit(mocker, agent: Agent):
    agent.legacy_config.google_api_key = "dummy-key"
    agent.legacy_config.google_custom_search_engine_id = "dummy-cse"
    mocker.patch(
        "eureka.commands.web_search.DDGS.text",
        side_effect=DuckDuckGoSearchException("Ratelimit"),
    )
    mock_google = mocker.patch(
        "eureka.commands.web_search.google", return_value='["https://example.com"]'
    )

    actual_output = web_search("test", agent=agent, num_results=1)

    assert actual_output == '["https://example.com"]'
    mock_google.assert_called_once_with("test", agent=agent, num_results=1)
